// Real installation/delivery/returns policy content, shared across the
// residential and commercial product detail pages (and, for delivery, the
// standalone /delivery-information page too) — rendered through
// BlogMarkdown (src/components/blog/blog-markdown.tsx) so it gets the same
// on-brand heading/list/table treatment as the blog, without re-building a
// bespoke renderer for it.

export const installationProcessMarkdown = `## Ocunio EV Solutions — Nationwide UK

Fast, compliant smart charger installations backed by certified OZEV, NICEIC and NAPIT engineers, with grant savings available on eligible installs.

### Standard Installation Package — £549 (inc. VAT) per chargepoint

The package includes:

- Up to 10 metres of high-grade EV ultra cable (compared to the 10m industry standard)
- 3 metres of neat protective exterior or interior trunking
- Type A RCBO with surge protection device (SPD)
- 3 to 4 hours of certified on-site installation and testing

Every job is backed by a 5-year workmanship guarantee, NAPIT-backed.

## The Ocunio 5-Step Customer & Installation Journey

1. **Order & itemised quote.** When you order a charger only from our website, we'll despatch it within 1 to 3 business days. Also at checkout, you can add professional installation. If you're applying for an OZEV grant, you'll pay nothing until your application is approved. To apply, you'll need a formal quote to submit with your OZEV application. You can generate this yourself directly from our website — a dated PDF showing an itemised breakdown of hardware, labour, VAT, and your grant amount. If you run into any trouble generating your quote, just email us, and we'll help.
2. **Digital home self-survey.** You'll receive a home survey link from OpenQuote to complete a guided photo survey — covering your consumer unit, gas and water pipes with bonding, your proposed charger location, and your parking bay. It takes under 10 minutes. Our experts will then review your survey to confirm it's a standard installation and arrange your installation date. If you'd prefer a physical site visit instead, we can arrange one on request, and it will be quoted separately.
3. **Engineering review & OZEV linkage.** We plan your electrical load. You then complete Stage 1 on Find a Grant, uploading our quote. Installation work begins only after written OZEV confirmation is received.
4. **Certified installation & 4-point photo audit.** Our engineer fits the charger and SPD and takes four statutory compliance photos: the charger close-up, serial/model badge, parking space relation, and wide building context.
5. **Commissioning & grant claim.** Your charger is tested and paired to your homeowner app. We handle evidence submission and claim your grant (£500 or 75%, whichever is lower) directly with OZEV.

## OZEV Grant Integration: Customer vs Installer Split

The OZEV grant process is a 2-stage model, split between what you complete and what we complete. It's important to understand this separation, as your side must be submitted and approved before we can finalise the claim.

| Stage 1: Consumer Portal (Find a Grant) | Stage 2: Ocunio Digital Claim Submission |
|---|---|
| Initiated and completed digitally by the tenant or flat owner | Ocunio opens the unique tracked link received via OZEV email |
| Enter Ocunio's registered OZEV installer number and business details | Inputs itemised financials matching the uploaded customer invoice |
| Upload Ocunio's dated, itemised installation quote | Enters the exact OZEV-approved hardware make, model, and serial number |
| Hard gate: await formal approval email before physical installation begins | Uploads the compiled 4-photo audit PDF; OZEV pays Ocunio directly |

## Transparent Rate Card & Optional Extras

The standard package covers most typical home installs. If your setup needs more, here's what applies:

| Specification / Boundary | Unit Price (inc. VAT) |
|---|---|
| Includes 1m cable, 3m trunking, SPD, labour and 5-year NAPIT guarantee | £549.00 |
| Pre-installation home inspection by a certified electrician | £120.00 inc. VAT |
| EV Ultra / armoured cable exceeding the 1m standard allowance | £15.00 / metre inc. VAT |
| Extra trunking beyond the 3m allowance | £8.00 / metre — inc. VAT |
| Non-standard cable routing, complex drilling, or consumer unit upgrades | £100.00 / hour — inc. VAT |
| Trenching, ducting, concrete post mounting for parking bays | Bespoke quote |

## Compliance & Accreditations

All installations comply with BS 7671:2018+A2:2022 (18th Edition IET Wiring Regulations, Section 722) and the Smart Charge Points Regulations (Schedule 1, cybersecurity ready).`;

export const deliveryPolicyMarkdown = `We are committed to providing fast, reliable, and secure delivery for all your EV charging equipment. In some cases, a third-party supplier may fulfil your order on our behalf.

## Delivery Coverage & Lead Times

- **Standard UK Delivery:** Orders are typically processed and dispatched within 1–3 working days.
- **Transit Time:** Once dispatched via our specialist couriers, UK deliveries usually arrive within 1–2 working days.

## Delivery Charges

| Order Value | Shipping Fee |
|---|---|
| Orders over £120 (ex. VAT) | FREE Standard UK Shipping |
| Orders under £120 (ex. VAT) | £10 Standard Carriage Fee |

**Special Services:** Need your order on a weekend? Saturday deliveries and expedited options can be arranged upon request.

**Custom Shipping Quotes:** For specialized delivery arrangements or international shipping, please email [info@ocunioenergy.com](mailto:info@ocunioenergy.com).

## Managing Your Delivery

- **Order Tracking:** Every shipment includes a delivery note detailing your order specifications.
- **Unexpected Delays:** While we work hard to meet delivery estimates, Ocunio Energy is not liable for delays caused by events outside our reasonable control or incomplete delivery instructions. If an unexpected delay occurs, we will notify you promptly and arrange a revised delivery date.
- **Failed Deliveries:** If delivery cannot be completed on the agreed date due to customer unavailability, goods will be stored securely. A reasonable fee may apply to cover re-delivery expenses.

## Receiving & Inspecting Your Goods

- **Product Inspection:** Customers are responsible for inspecting all goods upon arrival to ensure they are complete and in full working order before installation, integration, or resale to a third party.
- **Non-Delivery & Issues:** In the rare event an item is lost or fails to deliver, Ocunio Energy's liability is strictly limited to either replacing the missing goods within a reasonable timeframe or refunding the original purchase price.

## Specialist Delivery

A few product categories need extra care in transit:

- Portable power stations — allow up to 4 working days for delivery.
- Solar panels, battery storage, inverters, heat pumps and boilers — delivered via our pallet delivery service, priced at up to £110.

## Delivery Coverage

Northern Ireland, Scottish Highlands and Channel Islands — get in touch for a shipping quote.

## Need Help?

For returns, see the Returns tab above. For anything else, reach our team at [info@ocunioenergy.com](mailto:info@ocunioenergy.com) or via our [contact form](/contact-us).`;

export const returnsPolicyMarkdown = `Ocunio Energy (trading name of Nison Limited, Company Reg. No. 16371062, OZEV Installer No. 13528) supplies EV chargepoints and related equipment both as standalone mail order/online purchases and as part of a fully installed service. This policy sets out your rights under each purchase type and operates alongside — and does not affect — your statutory rights under the Consumer Contracts Regulations 2013 and Consumer Rights Act 2015.

## Part A — Mail Order & Online Purchases (Goods Only, Uninstalled)

This section applies where you have purchased equipment from Ocunio Energy for delivery, without an accompanying installation booking.

### A1. Right to Cancel

- You may cancel your order within 7 days of receiving the goods, without giving a reason.
- Once you notify us of cancellation, you have a further 7 days to return the item(s) to us.
- Proof of purchase is confirmed via your order confirmation or website transaction record.

### A2. Condition of Returned Goods

- Items must be unused, uninstalled, and returned complete with original packaging, accessories and documentation.
- A deduction may be made if the value of the goods has been reduced through handling beyond what is necessary to check them.

### A3. Processing & Refunds

- Refunds are issued to the original payment method within 14 days of receiving the returned goods (or evidence of return postage).
- Return postage is at your own cost unless the item is faulty or was sent in error.

## Part B — Installation Services

This section applies where Ocunio Energy has agreed to supply and install a chargepoint at your property.

### B1. Right to Cancel Before Work Begins

- You may cancel within 7 days of the contract date without charge, provided installation has not started.
- If equipment has already been ordered or an engineer visit scheduled ahead of your cancellation, reasonable costs already incurred may be deducted from any refund.

### B2. Cancelling Once Installation Has Started or Completed

- If you have given explicit written consent for installation to begin within the 14-day cooling-off period, your right to a full refund ends once the work is complete.
- Where installation is partially complete at the point of cancellation, a reasonable charge applies for work and materials already provided.

### B3. Grant-Funded Installations

- Any OZEV grant contribution already claimed and paid towards the installation is non-refundable to you where the installation is later cancelled or reversed after completion.

## Part C — Warranty & Faulty Equipment (Applies to Both Purchase Types)

- All chargepoints carry a minimum 12-month manufacturer's warranty against defects in materials or workmanship, in addition to your statutory rights.
- Report a suspected fault to Ocunio Energy before returning goods or arranging independent repair.
- For installed units, faults must be reported before any third party attempts repair, removal or relocation.
- Where a fault is confirmed, we will repair or replace the unit; if this is not possible, a refund or credit will be offered based on the depreciated value of the product.
- Warranty claims must include proof of purchase or installation date and a description of the fault.

## Part D — Exclusions

- Damage caused by misuse, incorrect operation, unauthorised relocation, or failure to follow the manufacturer's instructions.
- Faults arising from repair, modification or installation carried out by anyone other than Ocunio Energy or a NICEIC-approved engineer.
- Normal wear and tear, or damage from external causes (e.g. weather, power surges, vehicle collision).
- Faults arising from a specification, siting instruction or cable route supplied or insisted upon by the customer against our recommendation.
- Goods that have been personalised, custom-specified, or are not returned complete with an audit trail (order/return reference) linking them to your purchase.

## Part E — How to Return Goods or Raise a Claim

- Email [info@ocunioenergy.com](mailto:info@ocunioenergy.com) or call quoting your order or installation reference and address.
- We will confirm next steps in writing, including any collection or return arrangements and expected resolution timescale (typically up to 4–6 weeks for refund processing once an issue is confirmed).
- Where a card or account used for the original payment is no longer valid, we will contact you to arrange an alternative refund method.

## Part F — Your Statutory Rights

Nothing in this policy limits your statutory rights, including your rights under the Consumer Rights Act 2015 in respect of goods or installation services that are faulty, not as described, or not fit for purpose.`;

// Standalone "Terms and Conditions" and "Privacy Policy" pages (footer
// links). Written to cover the same real, business-specific practices and
// policies Ocunio Energy operates under — payment via Stripe, UK-based
// arbitration, GDPR/UK GDPR rights, etc. — in original wording, rather than
// reproducing the third-party legal-template boilerplate the previous site
// was built on.
export const termsAndConditionsMarkdown = `**Last updated:** ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}

These terms and conditions form a binding agreement between you and Nison Limited (trading as Ocunio Energy), company number 16371062, registered office 71-75 Shelton Street, Covent Garden, London, United Kingdom, WC2H 9JQ ("Ocunio Energy", "we", "us"). By accessing or using this website, or by purchasing products or installation services from us, you agree to be bound by these terms. If you do not agree, please do not use the site.

## 1. Eligibility

Our services are intended for users aged 18 and over. By using this site, you confirm that you meet this requirement and that any information you provide us is accurate and current.

## 2. Our Services

We sell EV chargepoints and related accessories, and provide OZEV-approved installation and grant-support services, as described elsewhere on this site. Product availability, specifications and pricing are subject to change without notice.

## 3. Intellectual Property

All content on this site — including text, graphics, logos, and software — is owned by or licensed to Nison Limited and is protected by UK and international intellectual property law. You may view, download and print pages for your own personal, non-commercial use only. You may not reproduce, republish, or exploit any part of the site for commercial purposes without our prior written consent.

## 4. Purchases and Payment

All prices are shown in British pounds sterling and, unless stated otherwise, include VAT. Payments are processed securely through Stripe; we do not store your full card details ourselves. We reserve the right to refuse or cancel an order, for example where a product is listed at an incorrect price or is out of stock.

## 5. Delivery and Returns

Delivery timescales are set out on our [Delivery Information](/delivery-information) page. Returns, cancellations and warranty claims for goods and installations are governed by the returns policy available on each product page.

## 6. Prohibited Activities

You agree not to misuse this site, including by: attempting to gain unauthorised access to our systems; using automated tools to scrape or harvest data; submitting fraudulent orders or payment details; or engaging in conduct that is abusive, unlawful, or disrupts the site for other users.

## 7. User Contributions

If you submit reviews, comments or other content to us (for example, via our contact form), you confirm it is your own and grant us a licence to use, reproduce and display it in connection with operating and promoting our services. We may remove any contribution at our discretion.

## 8. Third-Party Links

This site may link to third-party websites (for example, Royal Mail tracking, or the GOV.UK Find a Grant service). We do not control and are not responsible for the content or practices of those sites.

## 9. Liability

We provide the site and our services with reasonable care and skill, but we do not guarantee the site will always be available, uninterrupted or error-free. To the fullest extent permitted by law, we exclude liability for indirect or consequential loss. Nothing in these terms excludes or limits liability that cannot lawfully be excluded, including for death or personal injury caused by our negligence, or for fraud.

## 10. Termination

We may suspend or restrict access to the site for any user who breaches these terms.

## 11. Governing Law and Disputes

These terms are governed by the laws of England and Wales. Before starting formal proceedings, we encourage you to contact us so we can try to resolve any dispute informally. Where that isn't possible, the courts of England and Wales will have jurisdiction, without prejudice to any statutory right you have to bring proceedings in your own local courts.

## 12. Changes to These Terms

We may update these terms from time to time; the "Last updated" date above will reflect the most recent revision. Continued use of the site after a change constitutes acceptance of the updated terms.

## 13. Contact Us

Questions about these terms can be sent to [info@ocunioenergy.com](mailto:info@ocunioenergy.com) or via our [contact page](/contact-us).`;

export const privacyPolicyMarkdown = `**Last updated:** ${new Date().toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}

Nison Limited, trading as Ocunio Energy ("we", "us", "our"), is committed to protecting your privacy. This notice explains what personal information we collect, how we use it, and the rights you have over it.

## 1. Information We Collect

Depending on how you interact with us, we may collect: contact details (name, email, phone, address); order and payment information; documents you provide to support an OZEV grant application (such as proof of address or vehicle evidence); and technical information collected automatically when you browse the site (IP address, device and browser type, and cookie data).

## 2. How We Use Your Information

We use your information to: process orders and installation bookings; support your OZEV grant application; respond to enquiries; send service updates (and, where you've opted in, our newsletter); improve the site; and prevent fraud and misuse.

## 3. Payment Information

Card payments are processed by Stripe, a PCI-compliant third-party payment processor. We do not store your full card details on our own systems.

## 4. Sharing Your Information

We share information with trusted third parties only where necessary — for example, with OZEV as part of your grant application, with our couriers to deliver your order, or with Stripe to process payment. We do not sell your personal information to third parties.

## 5. Cookies

We use cookies and similar technologies to keep the site working correctly and to understand how it's used. You can control cookies through your browser settings.

## 6. How Long We Keep Your Information

We retain personal information only for as long as necessary to fulfil the purposes described in this notice, including any legal, accounting or reporting requirements.

## 7. Keeping Your Information Safe

We use appropriate technical and organisational measures to protect your information. However, no method of transmission over the internet or electronic storage is 100% secure, and we cannot guarantee absolute security.

## 8. Children's Privacy

Our services are not directed at children, and we do not knowingly collect personal information from anyone under 18.

## 9. Your Privacy Rights

If you are based in the UK, EEA or Switzerland, you have rights under GDPR/UK GDPR including the right to access, correct, or request deletion of your personal information, to object to or restrict certain processing, and to withdraw consent at any time. To exercise any of these rights, contact us using the details below.

## 10. Updates to This Notice

We may update this notice from time to time; the "Last updated" date above reflects the most recent revision.

## 11. Contact Us

For any questions about this notice, or to exercise your privacy rights, contact our team at [info@ocunioenergy.com](mailto:info@ocunioenergy.com) or 07525 567054.`;
